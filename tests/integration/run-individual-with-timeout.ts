...
async function runTestWithTimeout(name: string, testFn: () => Promise<any>, timeoutMs: number = 10000): Promise<TestResult> {
  ...
  return new Promise((resolve) => {
    const timeout = setTimeout(() => {
      ...
      resolve({ name, success: false, error: 'Test timed out', duration, timedOut: true });
    }, timeoutMs);
    ...
    testFn()
      ...
      .catch((error) => {
        ...
        resolve({ name, success: false, error: errorMessage, duration });
      });
  });
}

async function runAllTestsWithTimeout() {
  console.log('🚀 Starting individual endpoint tests with 10-second timeout...\n');
  ...
  for (let i = 0; i < tests.length; i++) {
    ...
    const result = await runTestWithTimeout(test.name, test.fn, 10000);
    results.push(result);
  ...
  if (failed > 0) {
    console.log('\n❌ Failed Tests:');
    results.filter(r => !r.success && !r.timedOut).forEach(r => {
  ...
  if (timedOut > 0) {
    console.log('\n⏰ Timed Out Tests:');
    results.filter(r => r.timedOut).forEach(r => {
      console.log(`  - ${r.name}`);
    });
  }
...
if (require.main === module) {
  runAllTestsWithTimeout().catch(console.error);
}

export { runAllTestsWithTimeout };
